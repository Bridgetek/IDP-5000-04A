...
void EVE_Mcu_initialize()
{
	...
	#define GPIO_PIN_RS485_EN 10
		sys_enable(sys_device_uart1);
		gpio_function(52, pad_uart1_txd);
		gpio_function(53, pad_uart1_rxd);
	
		gpio_dir(GPIO_PIN_RS485_EN, pad_dir_output);
		gpio_pull(GPIO_PIN_RS485_EN, pad_pull_none);
	
		gpio_write(GPIO_PIN_RS485_EN, 1);
		...
		uart_puts(UART1,
			...
			"--------------------------------------------------------------------- \r\n"
			"\r\n"
			" Welcome to ID50_FT900 The Project (ESD)... \r\n"
			"--------------------------------------------------------------------- \r\n"
			);
		init_printf(UART1, tfp_putc);
